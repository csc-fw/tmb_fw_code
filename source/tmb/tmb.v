...
//------------------------------------------------------------------------------------------------------------------
//	03/04/02 Initial
//	03/05/02 trigger mods
//	03/06/02 FFd CLCT inputs, unFF in Sequencer
//	03/07/02 minor signal name changes
//	03/17/02 FFd mpc_frame0,1 to improve timing
//	03/22/02 Fixed alct_vpf_tp, alct_vpf_tp
//	04/24/02 Shortened ALCT pipe by 1, based on FAST site test with CSC muons
//	04/26/02 Added allow_match
//	06/03/02 Added bx0_local, replaces bxn[1] in mpc frames
//	06/13/02 ALCT flood was keeping tmb_trig busy, so don't allow alct_only triggers here (now done in sequencer)
//	06/17/02 Another ALCT flood fix, removed alct_vpf_p2 from clct_only
//	07/03/02 Fixed alct latching, muon duplication, mpc injector, mpc frame order
//	07/08/02 Added mpc_response_ff and mpc_frame_ff for sequencer, mpc_delay now programmable
//	07/17/02 Now latch mpc_response on trigger or inject
//	07/24/02 Added mpc response RAM storage
//	08/01/02 Added ttc_mpc_inject
//	11/25/02 Updated clct format and mpc format
//	01/08/03 Renamed clct0_in to clct0_tmb, removed input FF stage
//	01/29/03 Pushed sync_err and bx0_local into clct
//	01/29/03 Rename alct0_in to alct0_tmb
//	01/29/03 Added startup blanking for mpc output
//	01/31/03 Replace alct shift registers with srl16e parallel shifter
//	02/04/03 Replace match logic and trigger latching, fix mpc_response_ff and mpc vme latching
//	02/26/03 Power up uses 2x clock for mpc_tx asyn set
//	03/02/03 Added match window location for off-line statistics
//	03/05/03 Mod bxn in mpc frame for clct_only or alct_only 
//	03/06/03 Fix tmb_busy if match happens on last_clct, fixed match window count
//	03/07/03 Fix clct_only and match win
//	04/30/03 Remove mpc_wait_done, only use mpc_response_ff, add mpc_accept_tp for scope
//	09/11/03 Remove FFs on mpc_accept and mpc_reserved, moved mpc_accept_tp and mpc_accept_vme to sequencer
//	03/15/04 Convert 80MHz inputs to ddr
//	03/15/04 Convert 80MHz outputs to ddr
//	04/20/04 Revert all DDR to 80MHz
//	06/04/04 Mod injector to work for 256 tbins, change to x_demux_v2 with aset for mpc_accept
//	06/07/04 Fix clct pipe -> clct real, so clct data is correct if alct matches in 1st clct window tick
//	06/10/04 Add programmable mpc_tx delay
//	10/04/04 Poobah demands ttc_bx0 sent to mpc instead of bx0_local
//	09/12/06 Mods for xst
//	09/25/06 More xst mods
//	10/04/06 Set ffs=0 init value. XST should be doing that, but does not
//	10/10/06 Replace 80mhz mux and demux with 40mhz ddr
//	10/23/06 Blanked 2nd muon mpc sync err and bxn if 2nd muon doesnt exist
//	04/09/07 Add mpc output enable via vme
//	04/27/07 Remove rx sync stage, shifts rx clock 12.5ns
//	04/30/07 Revert rx sync stage, because we can't shift mpc rx data by the necessary half cycle for ddr2
//	05/09/07 Update srl16e_bbl port names
//	05/21/07 Remove 2bx in mpc path, blank mpc quality if no valid lct
//	07/17/07 Convert to virtex2 RAMs, add state machine ascii display
//	07/18/07 Update injector RAM inits
//	08/10/07 Alter trig_pulse logic to always send pulse on match or when match window closes
//	08/23/07 Increase mxalct for full bxn
//	08/24/07 Remove bxn difference calc, same info is already available in header
//	10/19/07 Restructure clct0/1 add clctc to carry bits common to clct0/1
//	10/22/07 Add clctb buffer
//	10/24/07 Add wr_buf_adr to all sequencer replies
//	10/29/07 Replace clctb with wr_adr_xtmb
//	11/05/07 Blank sync err in 2nd lct if only 1 lct exists
//	01/17/08 Replaced lct quality with a plug-in module
//	01/18/08 Replace first,second with 0,1
//	02/28/08 Clean up bx0
//	02/29/08 Mod pipe adr
//	03/03/08 New alct matching pipeline
//	03/19/08 Resume pipeline after unwelcome time-wasting interruption
//	04/17/08 Replace pipeline logic with from tmb_match subdesign
//	04/18/08 Add indicator to prevent accidental compile with debug_mpc turned on
//	04/23/08 Blank empty LCTs except for bx0 heartbeat
//	04/29/08 Add mpc xmit counter signals to sequencer
//	05/09/08 Move xmpc frame storage to latch after mpc_tx_delay instead of before, makes bx0 delay independent
//	05/12/08 Move alct bx0 mux to srl input, add vme injector
//	05/19/08 Replace mpc rx pileline FFs with SRLs, remove mpc response ff
//	06/02/08 Add active feb list to clct data
//	07/14/08 Add Non-triggering readout mode
//	07/15/08 Nontrig bugfix
//	08/12/08 Add bx0_match
//	08/28/08 Add me1a clct blocking to mpc
//	09/04/08 Replace me1a blocking logic to remove combinatorial loop
//	09/05/08 Move me1a logic to output side of tmb FFs, cant meet timing constraint at FF inputs
//	09/12/08 Bugfix tmb was blocking LCTs to MPC for CLCTs on CFEB4 in normal CSCs
//	10/22/08 Conform signal names to sequencer output signals
//	11/15/08 Allow wr_avail_rtmb without trig_pulse
//	03/02/09 Add ecc error syndrome to alct data
//	04/24/09 Add mpc injector state machine recovery state
//	05/04/09 Add alct1,clct1 real time test points
...
// Sequencer
    ...
    output                  hmt_anode_alct_match;//anode hmt and alct vpf match
    output                  tmb_pulse_hmt_only; //hmt trigger only 
    output                  tmb_keep_hmt_only ;//hmt readout only
...
// VME Configuration
	...
	output					bx0_match2;			// ALCT bx0 and CLCT bx0 match in time
	...
	input					mpc_me1a_block;		// Block ME1A LCTs from MPC, but still queue for L1A readout
...
//------------------------------------------------------------------------------------------------------------------
  ...
  input run3_alct_df; // flag of run3 trigger data format
...
//------------------------------------------------------------------------------------------------------------------
    ...
    wire anode_hmt_fired_pipe   = run3_alct_df ? |alct0_pipe[13:12] : 1'b0;
    //wire [1:0] anode_intime_hmt = run3_alct_df ? (alct0_pipe[13:12] & {2{hmt_enable}}) : 2'b00;
    wire [1:0] anode_intime_hmt = run3_alct_df ? alct0_pipe[13:12] : 2'b00;
...
//------------------------------------------------------------------------------------------------------------------
	...
	wire [3:0]			clct_srl_ptr;
      ...
      wire [MXCCLUTB-1  : 0]  clct0_cclut_xtmb = {clct0_bnd_xtmb};
      wire [MXCCLUTB-1  : 0]  clct1_cclut_xtmb = {clct1_bnd_xtmb};
	...
	wire   wr_avail_xtmb_pipe = (clct_ptr_is_0) ? wr_avail_xtmb : wr_avail_xtmb_srl;
      ...
      assign {clct0_bnd_pipe} = clct0_cclut_pipe;
      assign {clct1_bnd_pipe} = clct1_cclut_pipe;
...
// Latch clct match results for TMB and MPC pathways
	...
	tmb_trig_pulse		<= trig_pulse || anode_hmt_fired_pipe;							// ALCT or CLCT or both triggered
	tmb_trig_keep_ff	<= trig_keep  || anode_hmt_fired_pipe;							// ALCT or CLCT or both triggered, and trigger is allowed
	tmb_non_trig_keep_ff<= non_trig_keep || anode_hmt_fired_pipe;						// Event did not trigger but is kept for readout
	...
	tmb_aff_list		<= clctf_pipe | ({MXCFEB{hmt_fired_only}});							// Active feb pipe
...
// CLCTs from ME1A
	...
	wire clct0_cfeb4	= clct0_real[15];					// CLCT0 is on CFEB4 hence ME1A
	wire clct1_cfeb4	= clct1_real[15];					// CLCT1 is on CFEB4 hence ME1A

	wire   kill_clct0	= clct0_cfeb4 && kill_me1a_clcts;	// Delete CLCT0 from ME1A
	wire   kill_clct1	= clct1_cfeb4 && kill_me1a_clcts;	// Delete CLCT1 from ME1A
...
// Decompose CLCT muons
	...
	wire	[2:0]	clct1_cfeb		= clct1[15:13];		// Key CFEB ID
      ...
      assign {clct0_bnd} = clct0_cclut;
      assign {clct1_bnd} = clct1_cclut;
//------------------------------------------------------------------------------------------------------------------
// LCT Quality
//------------------------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------------------------
      ...
      wire [9:0] clct1_xky_run3 = {clct1[15:8], 2'b00};
