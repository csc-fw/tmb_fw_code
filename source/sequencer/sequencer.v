...
//
// 	02/07/02 Ported from LCT99 AHDL
//	02/13/02 Added ext_inject
//	02/14/02 Added TTC counters
//	02/15/02 Trigger path pipeline delay tuning
//	02/19/02 Removed encode state to align sm with trigger path
//	02/21/02 Moved to Pentium 4 platform
//	02/21/02 Adjusted L1A counters, changed l1a to count all L1As, not just those that trigger
//	02/24/02 New clct_fifo logic
//	02/28/02 Delayed clct_fifo data wrt fifo_busy, so busy goes low 1 clock before data is done
//	03/01/02 Added DMB transmission RAM
//	03/01/02 Replaced library instances with behavioral code
//	03/02/02 Fixed RAM word count
//	03/03/02 Added ALCT RAM readout
//	03/07/02 Added TMB ports
//	03/07/02 New header format
//	03/08/02 Replaced header mux with parallel shifter
//	03/08/02 Removed vme_reset from raw hits RAM write-address counter
//	03/08/02 Put vme_reset back in, it's needed to keep VME from re-reading same event
//	03/08/02 Moved l1a_match to an xtmb latching header word
//	03/08/02 Fixed driver conflict
//	03/10/02 Changed marker frames
//	03/10/02 Started ALCT readout 1 clock earlier, delayed ALCT data 1 clock
//	03/10/02 Modified header bit assignments, stored header count in header frame, added firmware revcode
//	03/11/02 Changed revcode format
//	03/11/02 Delayed fifo_data 2 clocks wrt fifo_busy
//	03/12/02 Removed debug ports
//	03/12/02 Added clct_status array for CCB
//	03/12/02 Added led_alct
//	03/13/02 Added led_bd array
//	03/26/02 State machine outputs now go to on-board LEDs
//	03/26/02 ALCT-only mods, changed led_pretrig to led_clct
//	03/27/02 Fixed stat_invpat for alct-only, fixed DMB pin assignments, added sequencer_state for vme readout
//	03/28/02 Added alct_fifo_reset, was l1a before
//	04/02/02 Changed alct_fifo_reset to be xlast instead of idle
//	04/03/02 Pushed alct_start_read 2 clocks earlier to remove 2 words at start of alct readout
//	04/04/02 Output latched CLCTs to VME
//	04/09/02 Moved dmb_rx to ccb.v
//	04/24/02 Added scint_veto, removed header and alct from fifo sync mode
//	04/25/02 Added trig_keep
//	05/01/02 Changed ccb l1a request from ccb to start with tmb pre-trig
//	05/02/02 Fixed scint_veto FF
//	05/03/02 Removed keep_event, now L1A is just not issued for non-keep events, L1A can only be sent once per pretrig
//	05/06/02 L1A delay mux for alct_trig_pulse
//	05/07/02 Added veto oneshot, shifted l1a delay -2 for alct trigger
//	05/08/02 Tuned L1A delays for clct and alct so L1A is correct time after status 0 output
//	05/14/02 Fixed external trigger
//	06/03/02 Re-organized trigger input mux, add bx0_local
//	06/04/02 New revcode from VME
//	06/05/02 Trig mux fix
//	06/07/02 Added vme trigger source readback
//	06/10/02 LEDs now use VPFs
//	06/13/02 Added clct pattern trigger delay (didnt delay active_febs too, assume will use all_febs_active bit)
//	06/14/02 Delayed active_feb to be in time with delayed active_feb_flag
//	06/17/02 Removed clct pattern trigger delay, it causes a lot of problems, added l1a request delay for drift time
//	06/24/02 Added match requirement to pre-trigger, new trigger vector in headers 4,5
//	06/25/02 Revert to previous active_feb staging to improve speed
//	06/26/02 Add trig_source_noflush to keep external triggers and alct triggers, but prevent alct self-matching in tmb
//	06/27/02 Revert noflush because tmb really needs clct to request l1a
//	07/03/02 CLCT latching fixed
//	07/08/02 Delayed mpc frame latching from tmb processing stage
//	11/25/02 New cfeb patterns
//	12/02/02 Start mult-buffer mods
//	12/05/02 Add ranlct random active feb signals for DMB
//	12/09/02 Add force_active
//	12/16/02 New fifo controller signals
//	12/17/02 Block pretrigger if buffer not available, unless override enabled
//	12/19/02 Add to trigger ram data
//	01/22/03 Add alct vpf and clct active feb test points, separate alct_1st_valid and alct_active_feb triggers
//	01/29/03 Push sync_err and bx0_local into clct words
//	02/04/03 Tmb status mods
//	02/05/03 New clct sequencer and discard counters
//	02/06/03 L1A processing added
//	02/07/03 Scope now has trigger source select, added l1a stack
//	02/10/03 L1A push logic
//	02/11/03 Header unpacking
//	02/14/03 Readout format
//	02/18/03 Continued
//	02/19/03 Continued
//	02/20/03 Delay crc mux enables 1 clock
//	02/21/03 Add rd_feb_list for clct_fifo local readout
//	02/24/03 New clct_fifo
//	02/25/03 Resets for L1A RAM and dmb RAM address at power up
//	02/26/03 Debug
//	02/27/03 No progress, day wasted by OSU and Hauser
//	02/28/03 Advance wr_buf_pretrig by 2 clocks to minimize buffer waits
//	03/01/03 Separated CLCT trigger from TMB/MPC processing for speed, added turbo mode to bypass buffer waits
//	03/01/03 Add FMM trig_ready, programmable flush timer
//	03/02/03 Separated CLCT TMB MPC state machines
//	03/03/03 Fixed mpc response timeout, new parallel shifter for mpc response buffer number, deleted mpc machine
//	03/04/03 New bxn logic per Smith/Hauser/Varela
//	03/06/03 Fixed bxn for ovf and bx0 arriving at the same time
//	03/07/03 Readout section replaced with OSU headers
//	03/08/03 TMB and MPC timeouts prevent tmb_sm hang
//	03/09/03 Readout mods
//	03/09/03 Added missing e0c frame, retimed mod4, added status outputs, on-board LEDs
//	03/10/03 Fix to block start_read if no cfebs
//	03/11/03 Push alct_only, but mark as no-buffer events
//	03/12/03 Debugs removed
//	03/12/03 LEDs, state, peak buffer count
//	03/15/03 Scope clock changed from vme for speed, l1resets now syncronous, async was unstable
//	03/16/03 L1A LED fix
//	03/18/03 Add dmb_busy to scope
//	03/19/03 Bring in cfeb_exists from top level, un-reverse crc22, fix match window latch
//	03/25/03 Clip active_feb_flag to single pulse, add invp to scope
//	04/04/03 Mods for DMB data available signal at L1A instead of on readout pop
//	04/15/03 Mods for dmb_dav failure if 2nd l1a is 75ns after 1st l1a, bxn now latches at l1a
//	04/29/03 LHC_CYCLE now VME programmable, add ttc_bxreset to reset bxn but not l1a or buffers
//	04/30/03 Add mpc_accept_tp for scope display
//	05/07/03 New scope signals, correct drift delay counter for 1-clock longer cfeb_v8.v pipeline
//	05/08/03 Mods for scope64
//	05/09/03 Change nowrbuf discard count enable
//	05/12/03 Add cfeb info to header, add drift strobe to scope
//	05/13/03 Mods for scope96
//	05/14/03 Add buf_nbusy to scope, add alct raw hits sync
//	05/15/03 Add scope to dmb readout
//	05/16/03 Mods for scope readout
//	05/19/03 Add nrams to scope
//	06/04/03 Add nl1a flash on buffer full, programmable flash rate
//	09/10/03 Hacked for mpc_accept debug
//	09/11/03 Added mpc_accept_tp and mpc_accept_vme, moved here from tmb.v
//	09/12/03 Add allowpretrignoflush, unhacked mpc_accept debug
//	10/06/03 Mod for tmb2003a
//	05/10/04 Add RPC scope ports
//	05/18/04 Add RPC injector logic
//	05/19/04 Add RPC raw hits readout, increase header for RPC info
//	05/20/04 Conform RPC raws hits format to match CFEB
//	05/24/04 Skip RPC readout if all RPCs are disabled, but still send markers
//	06/09/04 Add last_header calculation to avoid underflow if r_nheaders =0
//	06/10/04 Add mpc_tx_delay
//	06/17/04 Add programmable scope trigger channel
//	07/23/04 Add nph_pattern to header
//	07/30/04 Remove crc reversed data, it was unused
//	08/02/04 Add event counters
//	08/03/04 Add tmb status counters to VME readout
//	08/05/04 Add mpc accept counters
//	08/26/04 Add counter stop if any overflows
//	10/01/04 Add scp_auto to header
//	10/04/04 Poobah demands ttc_bx0 instead of bx0_local sent to mpc
//	05/19/06 Add active feb flag blanking for disabled cfebs
//	07/26/06 Add prom state machine status to header data
//	07/27/06 Add uptime and board status to header
//	08/02/06 Add layer or trigger
//	08/04/06 Expand trigger source vector to 8:0
//	08/15/06 Remove ff from layer_trig signal to speed up by 1 clock
//	08/31/06 Output bxn to vme
//	09/12/06 Mods for xst
//	09/20/06 More xst mods
//	09/29/06 x_flash(11) becomes x_flashsm(19) for same 13ms flash
//	10/03/06 Mod ramlut calls
//	10/05/06 Replace for-loops with while-loops for xst
//	10/16/06 Add clct0 key and cfeb to scope
//	10/17/06 Add first_pat to scope
//	10/20/06 add l1a window position to header22
//	10/20/06 Fix l1a received counter
//	10/24/06 Mods to l1a window position logic
//	11/22/06 Mod scope channels
//	11/29/06 Remove debug scope channels, reorder hsds
//	02/12/07 Mod sequencer to set all_cfebs_active only for cfebs enabled for readout
//	04/06/07 Reduce RPC arrays from 2 to 4
//	04/27/07 Rename ttc_l1reset to ttc_resync
//	05/16/07 Go cylon in stop mode instead of flash, it looked too much like prom loading failures
//	05/21/07 Move cylon to vme, send flash signal to vme
//	06/20/07 Integrate layer trigger with pattern finder, remove ranlct
//	06/22/07 Increase tmb match timeout beco clct_width is temporarily 7bx
//	07/02/07 Revert timeouts until can check consequences in simulator
//	07/10/07 Increase tmb match timeout, but make clct_sm wait before re-arming for new event, else get wrong tmb buf
//	07/20/07 Reduce ram count
//	07/23/07 New scope.v has 512tbins, 1/2 fewer rams
//	07/31/07 Increase tmb match timeout again, extend count to 5 bits, to allow for clct_width=15 but not 16 (ie 0)
//	08/10/07 Mod tmb_sm to discard event on trig_pulse if not also trig_keep, don't push event on l1a stack either
//	08/13/07 Add buffer reset state machine, add buffer-reset counter
//	08/15/07 Restructure header per poobah, increase l1a_rx_counter from 4 bits to 12
//	08/20/07 Major reassignment of header bits, increase from 26 words to 32
//	08/22/07 Add bx0 counter, mod trailer logic for new format
//	08/27/07 Fix alct header section
//	08/29/07 Expand revcode, fix readout counter
//	09/04/07 More header mods
//	09/05/07 Yet more header mods
//	09/07/07 Fix header 22,37,38 + alct counter
//	09/10/07 Add tmb matching details to header
//	09/12/07 Latch tmb dupe signals using mpc data strobe, which is 1bx later than matching info
//	09/12/07 Took ff off of dupe signals, added no alct counter
//	09/14/07 Revert to normal scope channels
//	10/01/07 New clct fifo ports
//	10/03/07 Mod cfeb read start to begin after header instead of at header start
//	10/09/07 Conform crc logic to ddu, skip de0f marker because ddu logic fails to include it
//	10/15/07 Remove clct_fifo debug scope connections
//	10/16/07 Remove OR of 2 high order tbin bits in of raw hits stream, just let 4-bit tbin markers wrap at 16
//	10/18/07 Remove FF on LCT0/1 to save 1bx latency, add vpf blanking for clarity in simulator
//	10/19/07 Restructure clct0/1 and add new clcb to carry info common to both clcts
//	10/25/07 Replace ramluts with block rams
//	10/30/07 Add block ram address pipeline
//	10/31/07 Tune pipeline timing
//	11/01/07 Replace tmb timeout counters with tmb event counters
//	12/14/07 Replace buffer status signals with new buffer_write_ctrl
//	12/18/08 Add inhibit for auto buffer reset
//	12/19/07 Replace L1A stack
//	01/16/08 Delete L1A stack, replace entire L1A processing section
//	01/17/08 Repairs for sequencer-tmb_wrbuf subdesign simulation
//	01/18/08 Replace first,second with 0,1
//	01/22/08 Mod dmb shadow ram enables to avoid port conflicts in simulator
//	01/23/08 Replace buffer write control module
//	01/24/08 Limit buf pop to 1bx pulse, tune l1a offset to make pretrig bx correspond to l1a bx + l1a delay
...
//	02/26/08 Replace entire pre-trigger section with pretrig_unit.v import
//	02/29/08 New pre-trigger pipeline and header storage write-enables
//	03/03/08 Add l1a continuous write to header ram pipeline
//	04/18/08 New tmb.v connections clct_width changed to clct_window for consistency
//	04/18/08 Reorganize counters, add new counters from tmb.v
//	04/21/08 Change tp to tprt realtime test points
//	04/22/08 Add triad test point at input to raw hits RAMs, decrease drift delay pipe 1bx
//	04/23/08 Add clct_blanking to clear clcts with no hits
//	04/24/08 Make clct1 invalid if it has hits below threshold
//	04/25/08 Add independent rpc tbins to header
//	04/29/08 New event counter logic
...
//	05/19/08 Replace mpc rx pipeline in tmb.v, response pipe chained to tx delay pipe, fixes mpc response counter too
//	05/19/08 Mod alct stucture error detection to exclude wg=amu=q=bxn=0
//	05/23/08 Add lock_lost to sync_err
//	05/30/08 Conform powerup block, add startup state to readout machine to give buffer queue logic time to reset
//	06/02/08 Add lock_lost to header
//	06/03/08 Add received scope signal mux, mod active_feb mux to scope
//	07/14/08 Add non-triggering event keep
//	08/01/08 Gate r_nrpcs_read with rpc_read_enable to zero rpc count in header if rpc readout is off
//	08/23/08 Add csc orientation to header
//	08/28/08 Add me1a me1b pre-trig counters
//	09/11/08 Add trig keep to header41
//	09/17/08 Mod scope.v
//	09/18/08 Add scp no write to vme
//	09/19/08 Mod scp start to wait for rpc to finish
...
//	11/16/08 Mod l1a led to flash on any l1a push into queue
//	11/16/08 Mod l1a type logic
//	12/02/08 Add l1a look back
//	12/08/08 Mod wr_buf_ready for notmb L1A readouts
//	12/08/08 Add buffer status latches for vme debug register
//	12/10/08 Change counter enables[34:26] from wr_en_rtmb to wr_push_rtmb to avoid continuous counting in l1a-mode
//	02/24/09 Add ecc to data received from alct, add 2 ecc rx error counters
//	03/06/09 Shift counters up by 1 for new alct crc tx error counter
//	03/06/09 Add 5 separate cfeb pretrig counters
//	03/06/09 Add separate l1a bxn offset, a huge mistake in my opinion
//	03/11/09 Add alct counters for all ecc syndrome cases
//	04/24/09 Replace 5 dmb raw hits rams with 4 ram cascade to gain 1 ram, add recovery to state machines
//	04/27/09 Add dopa sump
//	04/30/09 Add perr miniscope
//	05/01/09 Add miniscope readout
//	05/05/09 Miniscope readout sm bugfix
//	05/06/09 Advance miniscope readout 1bx with lookahead logic
//	05/08/09 Send pre trig marker to rpc raw hits ram
//	06/06/09 Consolidate dmb_tx FFs into one section
...
//	08/25/09 Mod clct sm re-trigger to stay busy only for enabled trigger sources
//	09/14/09 Move sync error processing to sync_err_ctrl.v module
//	09/16/09 Add sync errors stop pretrig and l1a readout 
...
//	10/07/09 Fix record type
//	10/15/09 Remove empty bit in usrldrift
//	12/15/09 Add bad cfeb list to hdr30
//	02/10/10 Add event clear for vme diagnostic registers
//	03/07/10 Add blocked cfebs readout
//	04/16/10 Update read_sm_dsp states
//	04/29/10 Add bxn_sync_err to clct sync error
//	05/12/10 Mod bxn sync error logic pulse 1bx for counter and to fix sync_err always 1
//	05/27/10 Add inits to all reg declarations
//	05/27/10 Add widths to adder integers to conform to xst 12.1
//	05/28/10 Add debug outputs for postdrift stobes, increase l1a offset to full width
...
// TMB LCT Match
    ...
    hmt_anode_alct_match,//anode hmt and alct vpf match
    tmb_pulse_hmt_only, //hmt trigger only 
    tmb_keep_hmt_only ,//hmt readout only
...
// DMB Ports
	...
	input	[4:0]			dmb_tx_reserved;		// DMB backplane reserved
...
// TMB LCT Match
    ...
    input                  hmt_anode_alct_match;//anode hmt and alct vpf match
    input                  tmb_pulse_hmt_only; //hmt trigger only 
    input                  tmb_keep_hmt_only ;//hmt readout only
...
// Trigger/Readout Counter Ports
	...
	output  [MXCNTVME-1:0]  event_counter96;
	output  [MXCNTVME-1:0]  event_counter97;
	output  [MXCNTVME-1:0]  event_counter103;
	output  [MXCNTVME-1:0]  event_counter116;
	output  [MXCNTVME-1:0]  event_counter117;
// Event Counter Ports
...
// Pre-trigger counter, presets at evcntres or resync
...
// CLCT Pattern Trigger on hit cfebs, includes only cfebs with actual hits, and not overlaps from adjacent cfebs 
	...
	assign active_feb[4:0] = cfeb_active[4:0] | {MXCFEB{all_cfebs_active}};	// Active list includes boundary overlaps
...
// Trigger source was ME1A or ME1B
	wire only_me1a_hit =  active_feb_s0[4] && !(|active_feb_s0[3:0]);					// Only ME1A was hit
	wire only_me1b_hit = !active_feb_s0[4] &&  (|active_feb_s0[3:0]);					// Only ME1B was hit
...
//	wire [4:0] active_feb_list;		// Active FEB list selection
	...
	assign active_feb_list_pre = active_feb_s0[4:0] & {MXCFEB{active_feb_flag_pre}};
...
// On pretrigger push buffer address and bxn into the pre-trigger pipeline
	...
	assign pretrig_data[20:16]	= active_feb_list_pre[4:0];				// Active feb list at pre-trig
...
// Construct CLCTs for sending to TMB matching. These are node names only
	...
	assign clctf[4:0]	= aff_list_xtmb[4:0];		// Active feb list post drift
...
// Counter enable strobes
    ...
    //HMT counters, should start from counter96, from OTMB FW
    cnt_en[66] <= (|hmt_anode[1:0])  && ( hmt_anode[3:2] == 2'b00);// fire signal but not background region
    cnt_en[67] <= (|hmt_anode[1:0])  && (|hmt_anode[3:2]);// fire both signal and background region
    cnt_en[68] <= hmt_anode_alct_match;// hmt anode and alct match
    cnt_en[69] <= tmb_pulse_hmt_only; // tmb trigger pulse from hmt
    cnt_en[70] <= tmb_keep_hmt_only;  // tmb trigger keep from hmt
...
// Pre-trigger: store pre-trigger data in RAM mapping array
	parameter MXXPRE = 88;										// Pre-trig data bits
	wire [MXXPRE-1:0]	xpre_wdata;								// Mapping array
	wire [MXXPRE-1:0]	xpre_rdata;								// Mapping array

	assign xpre_wdata[4:0]		=	active_feb_list_pre[4:0];	// Active FEB list sent to DAQMB
	assign xpre_wdata[15:5]		=	trig_source_s0_mod[10:0];	// Trigger source vector
	...
	assign xpre_wdata[85]		=	wr_buf_avail;				// Write buffer is ready or bypassed
	assign xpre_wdata[86]		=	wr_buf_ready;				// Write buffer is ready
...
`ifdef RUN3DAQDATAFORAMT
	parameter MXXTMB = 38;		// Post drift CLCT data, 44+10
    ...
    //assign xtmb_wdata[47:38]    =   {clct1_bnd_xtmb, clct0_bnd_xtmb};
`else
...
`ifdef RUN3DAQDATAFORAMT
	parameter MXALCTD = 11+11+2+2;								// ALCT transmit frame data bits, 2alcts + bxn + tmb stats,26
...
// Unpack Pre-trigger data from RAM mapping array
	wire [4:0]	r_active_feb		=	xpre_rdata[4:0];	// Active FEB list sent to DAQMB
	wire [10:0]	r_trig_source_vec	=	xpre_rdata[15:5];	// Trigger source vector
	...
	wire		r_wr_buf_avail		=	xpre_rdata[85];		// Write buffer is ready or bypassed
	wire		r_wr_buf_ready		=	xpre_rdata[86];		// Write buffer is ready
...
// Unpack CLCT data sent to TMB from RAM mapping array
      wire [MXBNDB - 1   : 0] r_clct0_bnd_xtmb = 5'b0; // new bending
      wire [MXXKYB-1     : 0] r_clct0_xky_xtmb = {8'h00, 2'b10}; // new position with 1/8 precision
      wire [MXPATC-1     : 0] r_clct0_carry_xtmb = 0; // CC code
      wire [MXBNDB - 1   : 0] r_clct1_bnd_xtmb = 5'b0; // new bending
      wire [MXXKYB-1     : 0] r_clct1_xky_xtmb = {8'h00, 2'b10}; // new position with 1/8 precision
...
`ifdef RUN3DAQDATAFORAMT
    ...
    //assign {r_clct1_bnd_xtmb,  r_clct0_bnd_xtmb}  = xtmb_rdata[47:38];
`else
...
// Delay signals in pretrig region to come out at L1A time
	...
	srl16e_bit #(8,256) usrlbit (clock,l1a_delay,miniscope_data[j],miniscope_data_dly[j]);
	end
	endgenerate
...
// Output to DMB via IOB FFs, async load outputs nowrite mode until startup done, do not specify initial state else xst chokes
	...
	dmb_tx[39:35]	<= active_feb_list;		// DMB active cfeb list
	...
	dmb_tx[48:44]	<= dmb_tx_reserved[4:0];// Unassigned
	end
	end
...
// Front Panel: Normal Mode LED Digital pulse stretch for visual persistence
	x_flashsm #(19) uflashf0  (led_lct_ff,        led_hold, clock, led_lct_os        );	// LED0	Blue	LCT	
	x_flashsm #(19) uflashf1  (led_alct_ff,       led_hold, clock, led_alct_os       );	// LED1	Green	ALCT
	x_flashsm #(19) uflashf2  (led_clct_ff,       led_hold, clock, led_clct_os       );	// LED2	Green	CLCT
	x_flashsm #(19) uflashf3  (led_l1a_intime_ff, led_hold, clock, led_l1a_intime_os );	// LED3	Green	L1A
	x_flashsm #(19) uflashf4  (led_invpat_ff,     led_hold, clock, led_invpat_os     );	// LED4	Amber	INVP
	x_flashsm #(19) uflashf5  (led_nomatch_ff,    led_hold, clock, led_nomatch_os    );	// LED5	Amber	NMAT
	x_flashsm #(19) uflashf6  (led_nol1a_flush_ff,led_hold, clock, led_nol1a_flush_os);	// LED6	Red		NL1A
																						// LED7	Green	VME
...
//-------------------------------------------------------------------------------------------------------------------
	...
	clct_sump		|
	cfeb_tbin[4]	|
	tmb_alct_only_dly	|		// Not sure hot to implement this 4/28/08
	l1a_allow_alct_only	|		// Not sure hot to implement this
